/* =====================================================================
   OMBA401 · Quantitative Methods — student progress + monitoring.
   Named-student sync to the teaching-70f1c Realtime DB, under
   omba401/<sid>/...  Logs: who, when (first seen + last active),
   time spent (active seconds), per-section completion, and scores.
   Read by omba401/admin.html (auth-gated). Best-effort, fails silently.
...
(function(){
  ...
  function lk(k){return 'omba401_'+k+'_'+S.mod;}
  function mk(k,mod){return 'omba401_'+k+'_'+mod;}
  ...
  function loadLocal(){
    try{
      S.name=localStorage.getItem('omba401_name')||'';
      S.sid =localStorage.getItem('omba401_sid')||'';
      if(!S.name){var cn=getCookie('omba401_name'),cs=getCookie('omba401_sid');
        if(cn){S.name=cn;S.sid=cs||san(cn);try{localStorage.setItem('omba401_name',S.name);localStorage.setItem('omba401_sid',S.sid);}catch(e){}}}
  ...
  function backfillAll(){
    ...
    try{
      for(i=0;i<localStorage.length;i++){
        ...
        m=k&&k.match(/^omba401_(?:done|secs|score|meta)_(.+)$/);
        if(m)mods[m[1]]=true;
      }
    }catch(e){}
  ...
  function saveName(){
    ...
    try{localStorage.setItem('omba401_name',v);localStorage.setItem('omba401_sid',S.sid);}catch(e){}
    setCookie('omba401_name',v);setCookie('omba401_sid',S.sid);
