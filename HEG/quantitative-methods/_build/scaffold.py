...
SPEC=[
 dict(n=2,slug='Operations on functions',emoji='🧩',
  title='Cost, revenue, profit — <em>built from parts</em>',
  sub='Week 1 gave you one function at a time. A business never has one: it has a cost function <em>and</em> a revenue function, and the number the board cares about — profit — is what you get by <b>combining</b> them. This week is the arithmetic of functions: adding, subtracting, multiplying, dividing, composing and inverting.',
  ch='Operations on functions · composition · inverses',
  reads=[('Bonchek, "How to Create an Exponential Mindset" (HBR, 2016) — on Cyberlearn','#')],
  objectives=['Combine functions through addition, subtraction, multiplication, division and multiplication by a constant.',
              'Identify and use the main families of functions used in business.',
              'Build a profit function as <b>P = R − C</b>, and an average-cost function as a quotient.',
              'Compose functions — and read the composition as a chain of business steps.',
              'Introduce inverse functions and their properties, and say when an inverse exists at all.'],
  sections=[
   ('The hook','What does one more customer earn us?','Build P(q) from C(q) and R(q) and read it',
    ['Slider: set price and volume for XY Coffee\'s café; the three functions C, R and P are drawn on the same axes, live.',
     'The question the week answers: profit is not a number you are given, it is a function you <em>assemble</em>.'],'Hook'),
   ('The four operations','Adding and multiplying functions','(f+g), (f−g), (f·g), (f/g) and their domains',
    ['Worked: C(q) = 3 240 + 1.80q and R(q) = 4.50q give P(q) = R − C = 2.70q − 3 240.',
     'Widget: pick two functions from a menu and an operation; see the formula, the table and the graph of the result at once.',
     '⚠ The domain of a quotient is the trap: dividing by g kills every x where g(x) = 0. Average cost C(q)/q is undefined at q = 0 — and that is a fact about the business, not a technicality.'],'Build'),
   ('Price vs demand','Revenue is a product of two functions','R(p) = p · q(p), and why revenue is a parabola',
    ['Take the demand estimate q = 900 − 60p from Week 1 and build R(p) by multiplication.',
     'Widget: drag the demand line\'s slope and intercept; watch the revenue parabola change shape and its peak move.',
     'Classify (6 rows): which business quantity is a sum, which a product, which a quotient.'],'Combine'),
   ('Composition','A function inside a function','(f ∘ g)(x) = f(g(x)) — a chain of business steps',
    ['Worked: advertising spend → customers → revenue. Two functions chained give one answer, and the chain is the model.',
     'Widget: a two-stage machine — feed a number in, watch it pass through g and then f, with the intermediate value shown.',
     '⚠ f(g(x)) and g(f(x)) are different functions. Order is meaning: "10% discount then VAT" is not "VAT then 10% discount".'],'Chain'),
   ('Inverses','Running the machine backwards','f⁻¹, when it exists, and what it is for',
    ['Week 1\'s supply equation p = q/48 asked for q given p — that is the inverse, and it is the commonest real request.',
     'Widget: a function and its inverse drawn together with the line y = x, as the log and exponential were in Week 1.',
     'When does an inverse exist? Only if no two inputs share an output — the horizontal line test, and the reason it matters.'],'Invert'),
   ('Exponential growth and decay','A first model of sales and compounding','Growth as a function you can now combine',
    ['Revisit (1 + r)ⁿ from Week 1 and treat it as a function to be combined: revenue growing exponentially against costs growing linearly.',
     'The crossing point of those two curves is the question every founder asks, and it needs the logarithm of Week 1.'],'Growth'),
   ('Review quiz','Prove it to yourself','Twelve questions · not graded',
    ['The printed Session #2 review quiz, plus the business half: build a profit function, find the domain of a quotient, compose in the right order, and identify an inverse.'],'Quiz')],
  slides=['Quantitative Methods I · Session #2 — Operations on functions','Recap: one function at a time','Today: functions, combined','Adding and subtracting functions','Profit = revenue − cost, assembled','Multiplying functions: R(p) = p·q(p)','Dividing functions: average cost, and the hole at zero','The domain of a combination','Composition: f(g(x))','Order matters: discount then VAT, or VAT then discount','A chain of business steps, worked','Inverse functions','When does an inverse exist? The horizontal line test','Reading a supply curve backwards','Exponential growth and decay as a first model','Part 2 · exercises','Takeaways · homework · Week 3'],
  ex=[('Assemble a profit function',10,'C, R and P for the café, and the break-even read off P'),
      ('The four operations',12,'two functions, four combinations, four domains'),
      ('The quotient trap',10,'average cost: the domain, and why q = 0 is excluded'),
      ('Revenue as a product',14,'q = 900 − 60p → R(p), its roots and its peak'),
      ('Compose, in both orders',12,'f(g(x)) and g(f(x)) on the discount-and-VAT example'),
      ('Find the inverse',12,'three functions: invert them, and say which cannot be inverted'),
      ('Growth against linear cost',14,'when does exponential revenue overtake linear cost?')],
  tools=[]),
