/* =====================================================================
   HEG · Quantitative Methods I — presence, marked in the room.

   Attendance in Quantitative Methods is NOT graded — the syllabus is
   explicit: "Absences are not directly penalized. Attendance is monitored
   for pedagogical purposes." So the register here is a teaching
   instrument, not a grade component: it tells the instructor who was in
   the room when a topic was taught, which is what makes "the six people
   who missed Week 5 are the six who failed the midterm question on it"
   a thing you can see rather than guess. Same mechanism as HEG ·
   Applied Statistics, different stakes — and the student-facing copy
   must say so, or a student will believe their mark is at risk.

   It is still recorded in the room, at a moment the instructor controls,
   because a register nobody trusts is worse than no register.
   ...
   here: no open session, no column, no mark.
     ...
     QMPresence.mount({el, mod, label})   student button + instructor control
     QMPresence.summary(root)             {sessions:[…], byStudent:{…}}
   ===================================================================== */
window.QMPresence = (function () {
  "use strict";
  ...
  /* ---------- which window is open, for students ----------------------
     ...
     public pointer as well:
        ...
        qm1/_presence_now = { mod, label, open, ts }
  ...
  function mount(o){
    ...
    el.innerHTML='<h4>Presence · this session</h4>'+
      '<div class="pd" id="presWhy">Attendance is <b>not graded</b> in this course — it is recorded so your instructor knows who was in the room when a topic was taught. Your instructor opens this button in the room; press it once while it is red.</div>'+
  ...
  function hub(el){
    ...
    el.innerHTML=
      ...
      '<div class="pd">Attendance is <b>not graded</b> in this course — it is recorded for teaching, so your instructor can see which sessions you were in. Your instructor opens the window in the room; press the button once while it is red.</div>'+
    ...
    /* ---------- the fallback, for when `_presence_now` is unreadable -----
       The pointer node is how a student discovers WHICH session is taking
       marks: they cannot list `_presence` itself (that would hand out the
       cohort's sid list in one request, and a sid is what makes <ns>/<sid>
       guessable). If the pointer is missing — most likely because that
       rules block has not been deployed yet — presence must degrade, not
       break. So we ask each published session node in turn, one at a time,
       and take the first that reports itself open.

       It costs fifteen small requests instead of one, which is why it is
       throttled to once every 20 s however often pull() runs. Two
       consecutive total failures set `blocked`, which is what turns the
       student's message from "not open yet" into "this device cannot
       reach the register" — a distinction that matters in a room where
       somebody is waiting for a button to go red.
       -------------------------------------------------------------------- */
    function scan(){
      var t=Date.now();
      ...
      var found=null;
      return weeks.reduce(function(p,w){
        return p.then(function(){
          if(found)return;
          return fetch(node(w.mod)+'.json').then(function(r){return r.ok?r.json():null;})
            .then(function(k){if(k&&!k.error&&k.open)found={w:w,k:k};},function(){});
        });
      },Promise.resolve()).then(function(){
        scanning=false;
        if(found){
          fails=0;blocked=false;
          now={mod:found.w.mod,label:(found.k&&found.k.label)||found.w.name,open:true,ts:t};
          sess=found.k;
        }else{
          fails++;if(fails>=2)blocked=true;
          sess=null;
          if(now&&now.open)now={mod:now.mod,label:now.label,open:false,ts:t};
        }
        paint();
      },function(){scanning=false;fails++;if(fails>=2)blocked=true;paint();});
    }
    ...
    Promise.all(weeks.map(function(w){
      ...
      el.innerHTML='<div class="pres-mine">'+
        ...
        '<div class="pm-f">'+(!held
          ...
          ' Attendance is not graded — this is a register, not a mark. It counts only the sessions actually held.</div></div>';
    });
  }
