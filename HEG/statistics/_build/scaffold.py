...
SPEC=[
 ...
 dict(n=12,slug='How to use R',emoji='💻',
  title='Everything from Weeks 1–10, in <em>twelve lines</em>',
  sub='The exam is by hand; the job is not. This week installs R and RStudio and redoes the semester in code: describe the thirty prices, test the CHF 20 claim, compare the lake and the station, fit the price–rating line, run the chi-square — and get the same numbers you got with a calculator. Computer session; bring a laptop.',
  ch='Install R',
  reads=[('Install R','https://cran.r-project.org/'),('Install RStudio Desktop (free)','https://posit.co/download/rstudio-desktop/'),('R for Data Science — free online (Wickham, Çetinkaya-Rundel & Grolemund)','https://r4ds.hadley.nz/')],
  objectives=['Install R and RStudio and run a script.','Load a small dataset and compute mean, median, sd, quantiles; draw a histogram and a box plot.','Run t.test, prop.test, chisq.test and lm — and match each output to the hand calculation from earlier weeks.','Read an R output without panic: statistic, df, p-value, confidence interval.','Keep a script that reproduces your project\'s analysis.'],
  sections=[
   ('The hook','The same numbers','Predict which hand calculation R will disagree with',['A poll: "which of this semester\'s results will R get differently?" (None — but the rounding will surprise you.)'],'Hook'),
   ('Set-up','R, RStudio, the console','Vectors, data frames, reading a CSV',['Step-by-step install, with screenshots. The thirty prices as a vector; the pizzeria data as a data frame; <code>read.csv</code>.'],'Set-up'),
   ('Describe','Week 2, in R','mean · median · sd · quantile · hist · boxplot',['A script block per Week 2 section; compare each output to the widget from Week 2. The n − 1 shows up: <code>sd()</code> is the sample SD.'],'Describe'),
   ('Infer','Weeks 6–10, in R','t.test · prop.test · chisq.test · lm · aov',['One block per test, each against the hand result from its week. Reading <code>summary(lm(...))</code> line by line.'],'Infer'),
   ('Checkpoint','Prove it to yourself','Self-check quiz & where this goes next',['Eight questions: read four R outputs and answer the exam-style question each one settles.'],'Quiz'),
   ('Workshop','The pizzeria file · instalment twelve','Workbook fields',['w12_script — the R script that reproduces your project\'s numbers (paste it), and one line on what it told you that the hand calculation did not.'],'Exercise')],
  slides=['Applied Statistics · Week 12 — Working in R','Why code, when the exam is by hand','Install: R, RStudio','The console, vectors, data frames','Describe: the thirty prices','Infer: t.test, prop.test','chisq.test and lm','Reading an output without panic','Your script is your project','Takeaways · Weeks 13–15'],
  tools=[]),
