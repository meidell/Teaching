/* =====================================================================
   GBS · Finance Modelling (MIM specialization) — the course-progress
   ("mama") bar.  Ported from GBS/business-intelligence/courseprogress.js,
   which came from GBS/creator-economy; two modes:
   ...
   Reads local progress instantly (finmod_done_<mod>, written by
   /shared/progress.js), then refines from Firebase finmod/<sid>/mod when
   the student is signed in through /shared/login.js (finmod_auth).

   ⚠ THE MODULE IDS ARE THE SCHOOL'S WEEK NUMBERS, NOT 1…11 — the same
   decision the two sibling GBS courses took, and for the same reason: the
   ...
   student's saved progress is keyed on these ids, and the card in the hub
   says "Week 02 · Session 1" so both numbers are always on screen.
   ...
   denominator that does not exist. Each session is a two-hour build
   workshop whose page is a run sheet plus the capture widgets (the
   prediction, the model link, the check-row screenshot), NOT a slide
   deck, so the counts are small by design.
   ===================================================================== */
window.CourseProgress=(function(){
  var DB="https://teaching-70f1c-default-rtdb.europe-west1.firebasedatabase.app";
  var NS="finmod", K="finmod";
  var CHAPTERS=[
    {mod:'w2',  short:'W2', name:'Week 02 · The model nobody can audit',      total:6, live:false},
    {mod:'w3',  short:'W3', name:'Week 03 · Error-checking as architecture',  total:6, live:false},
    {mod:'w4',  short:'W4', name:'Week 04 · The three statements, linked',    total:6, live:false},
    {mod:'w5',  short:'W5', name:'Week 05 · Forecast, plug, circularity',     total:6, live:false},
    {mod:'w6',  short:'W6', name:'Week 06 · Free cash flow, honestly',        total:6, live:false},
    {mod:'w7',  short:'W7', name:'Week 07 · The number you would defend',     total:6, live:false},
    {mod:'w8',  short:'W8', name:'Week 08 · Sources, uses, and who pays',     total:6, live:false},
    {mod:'w9',  short:'W9', name:'Week 09 · Accretion or dilution',           total:6, live:false},
    {mod:'w10', short:'W10',name:'Week 10 · The LBO, on paper',               total:6, live:false},
    {mod:'w11', short:'W11',name:'Week 11 · Debt schedule, sweep, IRR',       total:6, live:false},
    {mod:'wp',  short:'PW', name:'Presentation week · Audit, decide, present',total:4, live:false}
  ];
  ...
  function caption(data){
    ...
    if(!live.length)return 'The term opens on Thursday 22 October. Each session unlocks on the morning it runs — there is nothing to read ahead, because what you take away is the model you build in the room.';
